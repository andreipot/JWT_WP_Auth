...
class Jwt_Auth_Public
{
    ...
    public function generate_token($request)
    {
        ...
        //Andrei's contribution to wordpress communitiy
        $body = json_decode($request->get_body());
        //$username = $body->username;//$request->get_param('username');
        $email = $body->email;//$request->get_param('username');
        ...
        $user = wp_authenticate($user_info->user_login, $password);

        /** If the authentication fails return a error*/
        if (is_wp_error($user)) {
            return new WP_Error(
                "jwt_auth_failed $user_info->user_login, $password",
                __('Invalid Credentials.', 'wp-api-jwt-auth'),
                array(
                    'status' => 403,
                )
